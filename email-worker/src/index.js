  ...
  return tracer.startActiveSpan('ES /orders/_doc', async (esSpan) => {
    try {
      ...
      if (err.meta && err.meta.statusCode === 404) {
        ...
        logger.warn(addTraceContext(esSpan, {
          message: `Order not found: ${orderId}`,
          order: { id: orderId },
      ...
      logger.error(addTraceContext(esSpan, {
...
async function sendEmail(order, span) {
  return tracer.startActiveSpan('send_confirmation_email', async (emailSpan) => {
    try {
      emailSpan.setAttribute('email.recipient', order.customerEmail);
      emailSpan.setAttribute('email.subject', `Your order #${order.id} has been confirmed`);
      ...
      logger.error(addTraceContext(emailSpan, {
...
async function processOrderConfirmed(msg, channel) {
  return tracer.startActiveSpan('process_order_confirmed_message', async (span) => {
    try {
    ...
    if (msg) {
      ...
      if (traceparent) {
        ...
        context.with(extractedContext, () => {
          processOrderConfirmed(msg, channel);
        });
      } else {
